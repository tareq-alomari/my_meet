    ...
    for (final report in reports) {
      if (report.type == 'inbound-rtp' &&
        ...
        if (packetsLost > 50) {
          ...
          AppLogger.warning(
              'جودة صوت ضعيفة للمستخدم $peerId: فقدان $packetsLost حزمة');
        } else if (jitter > 0.1) {
  ...
  Future<void> _handleRequest(HttpRequest req) async {
    try {
      ...
      if (_clients.length >= MAX_CLIENTS) {
        AppLogger.warning(
            'تم رفض اتصال جديد - تم الوصول للحد الأقصى من العملاء');
...
class _RoleSelectPageState extends State<RoleSelectPage> {
  ...
  Future<void> _navigateToManager() async {
    ...
    if (mounted) {
      Navigator.of(context)
          .push(MaterialPageRoute(builder: (_) => const ManagerPage()));
    }
  }
  ...
  Future<void> _navigateToParticipant() async {
    ...
    if (mounted) {
      Navigator.of(context)
          .push(MaterialPageRoute(builder: (_) => const ParticipantPage()));
    }
  }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow(
                        'حالة الخادم:', _serverRunning ? 'يعمل' : 'متوقف'),
                    if (_serverRunning)
                      _buildInfoRow(
                          'عدد العملاء المتصلين:', '${_server.clientCount}'),
            ...
            if (_isStarting)
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                        _serverRunning ? Colors.red[600] : Colors.green[600],
                    foregroundColor: Colors.white,
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                    ),
  ...
  Future<RTCPeerConnection> _createPeerConnection(String peerId) async {
    ...
    try {
      ...
      pc.onConnectionState = (RTCPeerConnectionState state) {
        ...
        _appendLog(
            'حالة الاتصال مع $peerId: ${_translateConnectionState(state)}');
